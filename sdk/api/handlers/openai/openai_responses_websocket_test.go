...
func TestForwardResponsesWebsocketCompletesImageGenerationCall(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		data := make(chan []byte, 2)
		errCh := make(chan *interfaces.ErrorMessage)
		data <- []byte(`{"type":"response.output_item.done","output_index":0,"sequence_number":4,"item":{"type":"image_generation_call","id":"img-1","status":"generating","result":"aW1hZ2U="}}`)
		data <- []byte(`{"type":"response.completed","response":{"id":"resp-1","output":[]}}`)
		close(data)
		close(errCh)

		completedOutput, _, _, errMsg, errForward := (*OpenAIResponsesAPIHandler)(nil).forwardResponsesWebsocket(
			ctx,
			newResponsesWebsocketWriter(conn),
			func(...interface{}) {},
			data,
		...
		if got := gjson.GetBytes(completedOutput, "0.status").String(); got != "completed" {
			serverErrCh <- fmt.Errorf("completed image status = %q, want completed; output=%s", got, completedOutput)
			return
		}
		serverErrCh <- nil
	}))
	...
	if errRead != nil {
		t.Fatalf("read image completion message: %v", errRead)
	}
	if got := gjson.GetBytes(completedPayload, "type").String(); got != "response.image_generation_call.completed" {
		t.Fatalf("first payload type = %q, want image completion; payload=%s", got, completedPayload)
	}
	...
	_, donePayload, errRead := conn.ReadMessage()
	if errRead != nil {
		t.Fatalf("read image output item: %v", errRead)
	}
	if got := gjson.GetBytes(donePayload, "item.status").String(); got != "completed" {
		t.Fatalf("image output item status = %q, want completed; payload=%s", got, donePayload)
	}

	_, responsePayload, errRead := conn.ReadMessage()
	if errRead != nil {
		t.Fatalf("read response completion: %v", errRead)
	}
	if got := gjson.GetBytes(responsePayload, "response.output.0.status").String(); got != "completed" {
		t.Fatalf("response output image status = %q, want completed; payload=%s", got, responsePayload)
	}
